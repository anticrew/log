...
func loadPaths(t *testing.T) (goPath, work string) {
	...
	goPath = winToUnix(os.Getenv("GOPATH"))
	require.NotEmpty(t, goPath)

	work, err := os.Getwd()
	require.NoError(t, err)

	return goPath, winToUnix(work)
}
	...
	testCases := map[string]testCase{
		"direct-skip-0": {
			...
			expected: runtime.Frame{
				...
				Line:     109, // where is Capture called
				Function: "github.com/anticrew/log/caller.Test_Capture.func1.1",
			},
		},
		"direct-skip-1": {
			fn:   Capture,
		...
		"inside-skip-0": {
			...
			expected: runtime.Frame{
				File:     work + "/capture_test.go",
				Line:     30, // where is Capture called
				Function: "github.com/anticrew/log/caller.inside",
			},
		},
		"inside-skip-1": {
			fn:   inside,
